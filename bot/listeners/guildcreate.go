...
func sendIntroMessage(ctx context.Context, worker *worker.Context, guild guild.Guild, userId uint64) {
	...
	msg := embed.NewEmbed().
		...
		AddField("Setup", "You can setup the bot using `/setup`, or you can use the [dashboard](https://tickets.tlgy.lol) which has additional options", false).
		AddField("Reaction Panels", fmt.Sprintf("Reaction panels are a commonly used feature of the bot. You can read about them [here](https://tickets.tlgy.lol/panels), or create one on [the dashboard](https://tickets.tlgy.lol/manage/%d/panels)", guild.Id), false).
		...
		AddField("Tags", "Tags are predefined tickets of text which you can access through a simple command. You can learn more about them [here](https://tickets.tlgy.lol/tags).", false).
		AddField("Claiming", "Tickets can be claimed by your staff such that other staff members cannot also reply to the ticket. You can learn more about claiming [here](https://tickets.tlgy.lol/claiming).", false).
