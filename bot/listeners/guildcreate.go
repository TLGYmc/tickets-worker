...
func sendIntroMessage(ctx context.Context, worker *worker.Context, guild guild.Guild, userId uint64) {
	...
	msg := embed.NewEmbed().
		SetTitle("Tickets").
		SetDescription("Thank you for inviting Tickets to your server! Below is a quick guide on setting the bot up, please don't hesitate to contact us in our [support server](https://discord.gg/KSaBtRsAtx) if you need any assistance!").
		SetColor(customisation.GetColourOrDefault(ctx, guild.Id, customisation.Green)).
		AddField("Setup", "You can setup the bot using `/setup`, or you can use the [dashboard](https://tickets-dashboard.astro-mc.xyz) which has additional options", false).
		AddField("Reaction Panels", fmt.Sprintf("Reaction panels are a commonly used feature of the bot. You can read about them [here](https://tickets-dashboard.astro-mc.xyz/panels), or create one on [the dashboard](https://tickets-dashboard.astro-mc.xyz/manage/%d/panels)", guild.Id), false).
		...
		AddField("Tags", "Tags are predefined tickets of text which you can access through a simple command. You can learn more about them [here](https://tickets-dashboard.astro-mc.xyz/tags).", false).
		AddField("Claiming", "Tickets can be claimed by your staff such that other staff members cannot also reply to the ticket. You can learn more about claiming [here](https://tickets-dashboard.astro-mc.xyz/claiming).", false).
		AddField("Additional Support", "If you are still confused, we welcome you to our [support server](https://discord.gg/KSaBtRsAtx). Cheers.", false)
